...
print(lista_diagonal)
_punto_matriz = [0,0,0,0]
convergence = False
print(matriz_df)
def jacobs(matriz, punto, diagonal, independientes, cuenta, error):
    ...
    for row in range(matriz.shape[0]):
        ...
        for col in range(matriz.shape[1]):
            if (col < matriz.shape[1]-1):
                ...
                res1 += matriz.iloc[row,col] * punto_anterior[col]
...
cuenta = 0
list_error = []
print('PUNTO => ', _punto_matriz)
while cuenta < iteraciones:
    punto, list_error = jacobs(matriz_df, _punto_matriz, lista_diagonal, lista_independientes, cuenta, list_error)
    print('PUNTO => ', punto)
    row_count += 1
    cuenta += 1
